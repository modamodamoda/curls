...
const grammars = {
    'default': [ 
        { name: 'BLOCK', 
          grammar: ['BLOCK', '?END', '$ARG', 1] },
        { name: 'BLOCKPART', 
          grammar: ['PARTIAL', 'BLOCK', '?END', '$ARG', 1] },
        { name: 'IF', 
...
function parse(orig, block, start = 0, end = null, state = false) {
        ...
        while(idx < available_grammars.length) {
            ...
            for(let rule of Array.isArray(grammar.grammar[grammar.gPos]) ? grammar.grammar[grammar.gPos] : [grammar.grammar[grammar.gPos]]) {
                if(typeof rule === 'object') {
                    if(tok == rule.start) {
                        [argPos, argRes, argGr] = parse(orig, block, pos + 1, [rule.end], rule.state.substr(1));
                        if(argPos != -1) {
                    ...
                    [argPos, argRes, argGr] = parse(orig, block, pos, null, rule.substr(1));
                    if(argPos != -1) {
                        tok = rule.substr(1);
                        break;
                    }
            ...
            else if(tok == 'ENDBLOCK' || tok == 'BLOCK') {
