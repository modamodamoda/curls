...
const ctokens = { // fixed tokens
    ...
    CREATEPART: ['#partial'],
    IF: ['#if'], // if
    ENDEXPR: ['/each', '/if', '/with'], // end of logical expressions
...
function isExprBegin(char) {
    return char == '#';
}

function isEndExprBegin(char) {
    return char == '/';
}
    ...
    while(i < str.length) {
        if(!parsing) {
            ...
            if(tokens.length == 0 || (rawhtml && tokens.length == 1)) {
                if(isEndExprBegin(str[i])) {
                    i++;
                    [i, t] = process_expr(str, i);
                    tokens.push(['ENDEXPR', t, i]);
                    continue;
                } else if(str[i] == '>') {
                    tokens.push(['PARTIAL']);
                    i++;
